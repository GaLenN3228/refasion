...
//      _showMessage(context, const Text('Location permission was NOT granted'));
  ...
  void showPickPoints() {
    if (pickPointRepository.isLoaded) {
      Future.delayed(const Duration(milliseconds: 2500), () {
        pickPointRepository.pickPointResponse.content
          ...
          final Placemark _placemark = Placemark(
            point: _point,
            opacity: 0.8,
            iconName: 'assets/images/png/pick_point.png',
            onTap: (double latitude, double longitude) =>
                print('Tapped me at $latitude,$longitude'),
          );
  ...
  void showUserLayer() async {
    if (_permissionStatus == PermissionStatus.granted) {
      await controller.showUserLayer(
          iconName: 'assets/images/png/place.png',
          arrowName: 'assets/images/png/place.png',
